...
class _ManageInspectionScreenState extends State<ManageInspectionScreen> {
  ...
  Future<void> _runInspection() async {
    ...
    final result = await inspectionProvider.performInspectionForCollege(
      college: _selectedCollege!,
      department: _selectedDepartment,
      dynamicRequirements: requirementsProvider.requirements,
    );
...
DropdownButtonFormField<Department>(
                ...
                decoration: InputDecoration(labelText: 'Select Department', hintText: _selectedCollege == null ? 'Select College First' : 'Select Department'),
                selectedItemBuilder: (BuildContext context) {
                  final departments = _selectedCollege == null ? [] : departmentProvider.getDepartmentsForCollege(_selectedCollege!.id);
                ...
                items: _selectedCollege == null ? [] : departmentProvider.getDepartmentsForCollege(_selectedCollege!.id).map((department) {
                  final displayName = department.subSelectionType != null 
                      ? "${department.name} (${department.subSelectionType})"
                      : department.name;
                ...
                onChanged: _selectedCollege == null ? null : (department) {
                  setState(() {
                ...
                validator: (value) {
                  if (_selectedCollege == null) {
                    return 'Please select a college first';
                  }
              ...
              children: [
                Expanded(
                  child: ElevatedButton(
                    onPressed: (_selectedCollege != null && _selectedDepartment != null && !_isLoading) ? _runInspection : null,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text('View Master Equipment List'),
              ),
            ),
            const SizedBox(height: 16),
            _buildReferenceLink(
              ...
              Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Inspection Results for ${_selectedCollege?.name ?? 'Unknown College'}',
